...
public class Glider : MonoBehaviour {
	...
	void FixedUpdate() {
		...
		rb.AddTorque((transform.up * rud - transform.forward * ail) * Input.GetAxis("Horizontal"));
		rb.AddTorque(transform.right * el * Input.GetAxis("Vertical"));
	...
	Vector3 aeroForce() {
		...
		float D = cd * 0.5f * rho * vel_b.sqrMagnitude * cord * span;
